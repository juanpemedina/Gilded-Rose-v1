defmodule GildedRose do
  ...
  defp handle_expired_item(%{name: "Sulfuras, Hand of Ragnaros"} = item), do: item

  defp handle_expired_item(item) when item.quality > 0, do: decrease_quality(item)
  defp handle_expired_item(item), do: item
  ...
  def update_item(item) do
    item = cond do
      ...
      item.name != "Aged Brie" && item.name != "Backstage passes to a TAFKAL80ETC concert" -> decrease_quality(item)
      true -> item
    end
